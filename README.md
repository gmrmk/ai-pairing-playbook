...
The `ai-pairing-playbook` Claude Code skill â€” a reciprocal
...
2. **Three flavors of "proceed"** â€” disambiguate authorization
   for state-mutating steps.
3. **Name auto-mode boundaries** â€” narrate where auto ends.
4. **Stage multi-part thoughts** â€” type the whole thought in one
   send, or signal "hold."
5. **Make visual reports diagnostic** â€” include what *doesn't*
   happen at the end of an observed sequence.
6. **Prove the trade-off on a copy** â€” on a high-blast-radius
   request, test it in isolation before complying; neither
   capitulate nor stonewall.

Plus a bonus: **texture beats brevity for corrections** â€” the
...
(The doubled `ai-pairing-playbook` is intentional â€” the outer
...
tech stack and domain. Update it as new mechanics surface â€” this
is a working document, not a fixed ruleset.

The 6 shifts are stable across backends; the location of the
partner-notes doc varies. See `SKILL.md Â§ Backend portability`
...
through a four-step funnel (state claim â†’ name second signal â†’
verify â†’ act). Hedged or generic prompt advice is itself the
failure mode this skill exists to prevent â€” session-grounded
specifics (*"here's the moment X would have prevented"*) beat
generic best-practices every time.

## Companion skill

**dual-log-memory** â€” A memory architecture pairing a fix log
...
â†’ https://github.com/gmrmk/dual-log-memory
...
the addition. The skill grows by its own discipline â€” driven by
